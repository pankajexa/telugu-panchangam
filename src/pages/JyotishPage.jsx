...
export default function JyotishPage() {
  ...
  return (
    <div style={styles.container}>
      <h1 style={{ ...styles.title, fontFamily: font, color: colors.text }}>{t('jyotish.title')}</h1>
      <p style={{ ...styles.subtitle, color: colors.textMuted }}>{t('jyotish.subtitle')}</p>

      {/* Today's cosmic snapshot */}
      <div style={{ ...styles.todayCard, background: colors.cardBg, border: `1px solid ${colors.border}` }}>
        ...
        <div style={styles.todayGrid}>
          {todayData?.tithi && (
            <div style={styles.todayItem}>
              ...
              <span style={{ ...styles.todayValue, fontFamily: font, color: colors.text }}>{pick(todayData.tithi.name, todayData.tithi.nameEn)}</span>
            </div>
          )}
          {todayData?.nakshatra && (
            <div style={styles.todayItem}>
              <span style={{ ...styles.todayLabel, fontFamily: font }}>{t('jyotish.nakshatra')}</span>
              <span style={{ ...styles.todayValue, fontFamily: font, color: colors.text }}>{pick(todayData.nakshatra.name, todayData.nakshatra.nameEn)}</span>
            </div>
          )}
          {todayData?.yogam && (
            <div style={styles.todayItem}>
              <span style={{ ...styles.todayLabel, fontFamily: font }}>{t('jyotish.yogam')}</span>
              <span style={{ ...styles.todayValue, fontFamily: font, color: colors.text }}>{pick(todayData.yogam.name, todayData.yogam.nameEn)}</span>
            </div>
          )}
          {todayData?.karanam && (
            <div style={styles.todayItem}>
              <span style={{ ...styles.todayLabel, fontFamily: font }}>{t('jyotish.karanam')}</span>
              <span style={{ ...styles.todayValue, fontFamily: font, color: colors.text }}>{pick(todayData.karanam.name, todayData.karanam.nameEn)}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={styles.rashiGrid}>
        {rashiList.map((rashi) => (
          <div key={rashi.english} style={{ ...styles.rashiCard, background: colors.cardBg, border: `1px solid ${colors.border}` }}>
            <span style={styles.rashiSymbol}>{rashi.symbol}</span>
            <span style={{ ...styles.rashiPrimary, fontFamily: font, color: colors.text }}>{pick(rashi.telugu, rashi.english)}</span>
...
const styles = {
  ...
  subtitle: { fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontSize: '12px', textAlign: 'center', margin: '0 0 20px' },
  todayCard: {
    borderRadius: '16px',
    padding: '16px',
    marginBottom: '24px',
  ...
  todayValue: { fontSize: '13px', fontWeight: 700 },
  ...
  rashiPrimary: { fontSize: '12px', fontWeight: 700 },
