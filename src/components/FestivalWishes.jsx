...
function buildWishText(message, festivalName, language) {
  const header = language === 'te'
    ? `*శ్రీ పరాభవ నామ సంవత్సర ${festivalName} శుభాకాంక్షలు*`
    : `*Sri Parabhava Nama Samvatsara ${festivalName} wishes*`;
...
export default function FestivalWishes({ festival }) {
  ...
  const handleSelect = useCallback(async (template) => {
    ...
    const name = language === 'te' ? festival.telugu : festival.english;
    const fallbackText = buildWishText(template.message, name, language);

    setGenerating(true);
    try {
      const blob = await generateWishCard(festival, template);
      await shareImageWithFallback(blob, `${festival.english}-wishes.png`, fallbackText);
    } catch (_) {
