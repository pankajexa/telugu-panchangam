...
function buildShareText(data) {
  ...
  lines.push(`   ${formatNakDT(data.nakshatra.start)} — ${formatNakDT(data.nakshatra.end)}`);
  lines.push(``);

  lines.push(`🕉 యోగం: ${data.yogam.name} | కరణం: ${data.karanam.name}`);
  lines.push(``);

  lines.push(`⚠ రాహుకాలం: ${data.rahuKalam}`);
  lines.push(`⚠ వర్జ్యం: ${data.varjyam}`);
  lines.push(`⚠ దుర్ముహూర్తం: ${data.durmuhurtham}`);
  lines.push(``);
...
export default function ShareButton({ data }) {
  const handleShare = useCallback(() => {
    ...
    // Try native share API first (works on mobile, lets user choose app)
    if (navigator.share) {
  ...
  return (
    <button style={styles.button} onClick={handleShare}>
      ...
      <span style={styles.text}>పంపండి</span>
    </button>
  );
}
